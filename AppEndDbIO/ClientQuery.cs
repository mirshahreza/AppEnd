...
namespace AppEndDbIO
{
	public class ClientQuery : IDisposable
    {
        ...
        public JsonElement? Data { set; get; }
        ...
        private void Init(ClientQuery clientQuery)
        {
            ...
            // Convert Data to Params if Data exists and Params is null or empty
            if (clientQuery.Data.HasValue && (Params == null || Params.Count == 0))
            {
                Params = [];
                if (clientQuery.Data.Value.ValueKind == System.Text.Json.JsonValueKind.Object)
                {
                    foreach (var prop in clientQuery.Data.Value.EnumerateObject())
                    {
                        object? value = null;
                        switch (prop.Value.ValueKind)
                        {
                            ...
                            case System.Text.Json.JsonValueKind.Number:
                                if (prop.Value.TryGetInt32(out int intVal))
                                    value = intVal;
                                else if (prop.Value.TryGetInt64(out long longVal))
                                    value = longVal;
                        ...
                        Params.Add(new ClientParam(prop.Name, value));
                    }
                }
            }
        ...
        private string GetCreateStatement()
        {
            ...
            foreach (DbQueryColumn dbQueryColumn in dbQuery.Columns)
            {
                ...
                {
					// Skip identity columns - they are auto-generated by SQL Server
					// First check if this is the primary key and if it's identity
					if (dbQueryColumn.Name.EqualsIgnoreCase(pk.Name) && pk.IsIdentity)
					{
						continue;
					}
					
					// Also check from dbDialog columns
					DbColumn? dbColumn = dbDialog.TryGetColumn(dbQueryColumn.Name);
					if (dbColumn != null && dbColumn.IsIdentity)
					{
						continue;
					}
					
					// Additional safety: Always skip primary key column named "Id" if it's the primary key
					// This handles cases where IsIdentity might not be set correctly in dbdialog.json
					// but the column is actually identity in the database (common pattern)
					if (dbQueryColumn.Name.EqualsIgnoreCase("Id") && pk.IsPrimaryKey && pk.Name.EqualsIgnoreCase("Id"))
					{
						// Skip Id column - it's likely identity even if not marked as such in dbdialog.json
						// The database will auto-generate the value
						continue;
					}
        ...
        private string GetCreateStatementForHistory(List<DbQueryColumn>? columnsToInsert, string masterTable, string masterTablePkName, string masterTablePkParamName)
        {
            ...
            foreach (DbQueryColumn dbQueryColumn in dbQuery.Columns)
            {
                ...
                // Skip identity columns - they are auto-generated by SQL Server
                // Check both from dbDialog and from pk (primary key) to be safe
                if (dbQueryColumn.Name.EqualsIgnoreCase(pk.Name) && pk.IsIdentity)
                {
                    continue;
                }
		...
		private void CalculateSharpParams()
        {
            ...
            foreach (DbParam dbParam in dbQuery.Params)
            {
                if (dbParam.ValueSharp is not null)
                {
					...
					{
						obj = System.Guid.NewGuid().ToString();
					}
                    dbParam.Value = obj;
                }
            }
