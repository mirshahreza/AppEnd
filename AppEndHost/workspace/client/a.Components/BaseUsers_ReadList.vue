					...
					<table class="table table-sm table-hover w-100 ae-table m-0 bg-transparent">
						<thead>
							<tr class="d-none d-md-table-row d-lg-table-row d-xl-table-row">
								...
								<th class="sticky-top ae-thead-th text-center" style="width:40px;" data-ae-actions="DefaultRepo.BaseUsers.DeleteByKey"></th>
							</tr>
						</thead>
						<tbody v-if="initialResponses[0].IsSucceeded===true">
							<tr v-for="i in initialResponses[0]['Result']['Master']">
								<td class="ae-table-td text-dark text-center" @click="openById({compPath:'/a.Components/BaseUsers_UpdateByKey',recordKey:i.Id,refereshOnCallback:true,actionsAllowed:'DefaultRepo.BaseUsers.UpdateByKey',fkToParent:''});">
										...
										<tbody>
											<tr>
												...
												<td class="fb align-middle text-danger">
													<div>{{shared.formatDate(i["LoginTryFailLastOn"])}} <span class="fw-bold">[ {{shared.fixNull(i["LoginTryFailsCount"],'-')}} ]</span></div>
												</td>
											</tr>
											<tr>
												<td class="align-middle text-success" style="width:45px;">{{shared.translate("Seccess")}}</td>
												<td class="fb align-middle text-success">
													<div>{{shared.formatDate(i["LoginTrySuccessLastOn"])}} <span class="fw-bold">[ {{shared.fixNull(i["LoginTrySuccessesCount"],'-')}} ]</span></div>
												</td>
											</tr>
										</tbody>
									</table>
								</td>
								<td class="ae-table-td pointer" @click="openById({compPath:'/a.Components/BaseUsers_IsActiveUpdate',recordKey:i.Id,refereshOnCallback:true,actionsAllowed:'DefaultRepo.BaseUsers.IsActiveUpdate',fkToParent:''});">
									<div class="input-group input-group-sm bg-hover-primary rounded-2 p-2">
								...
								<td class="ae-table-td pointer" @click="openById({compPath:'/a.Components/BaseUsers_LoginLockedUpdate',recordKey:i.Id,refereshOnCallback:true,actionsAllowed:'DefaultRepo.BaseUsers.LoginLockedUpdate',fkToParent:''});">
									<div class="input-group input-group-sm bg-hover-primary rounded-2 p-2">
										<div class="input-group-text rounded-2 me-1">
								...
								<td></td>
								<td class="ae-table-td text-center text-secondary text-hover-danger pointer" data-ae-actions="DefaultRepo.BaseUsers.DeleteByKey" @click="deleteById({pkValue:i.Id})">
...
<script>
	shared.setAppTitle(shared.translate("BaseUsers, ReadList"));
